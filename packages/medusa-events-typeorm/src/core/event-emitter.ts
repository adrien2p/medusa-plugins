...
class CustomEventEmitter extends EventEmitter {
	 ...
	 */
	public register<T>(eventName: string | symbol, propertyName: string, classType: Type<T>): void {
	...
	public registerListeners(container: AwilixContainer): void {
		for (const [, listenerDescriptors] of [...this.#listeners]) {
			for (const listenerDescriptor of listenerDescriptors) {
				...
				const listenerHandlerName = `${eventName.toString()}${classType.name}${propertyName}`;
				...
				if ((!lifeTime || lifeTime === Lifetime.SINGLETON) && alreadyRegisteredListenerHandlerIndex !== -1)
					continue;
...
export function attachSubscriber<T extends Constructor<EntitySubscriberInterface<unknown>>>(
	Subscriber: T,
	transactionalEntityManager?: EntityManager
): void {
	const subscriberIndex = dataSource.subscribers.findIndex((subsciber: EntitySubscriberInterface) => {
		return subsciber.constructor.name === Subscriber.name;
	});

	const subscriberReplacement = new Subscriber(transactionalEntityManager);
	if (subscriberIndex < 0) {
		// Add subscriber
		dataSource.subscribers.push(subscriberReplacement);
	} else {
		// Replace subscriber
		dataSource.subscribers.splice(subscriberIndex, 1, subscriberReplacement);
	}
}
